...
// ----------------------------
// Editor
static int imgui_BeginNodeEditor(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
static int imgui_EndNodeEditor(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
// Node
static int imgui_BeginNode(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
static int imgui_EndNode(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    ImNodes::EndNode();
...
// Input
static int imgui_BeginInputAttribute(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
...
static int imgui_EndInputAttribute(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
// Output
static int imgui_BeginOutputAttribute(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
...
static int imgui_EndOutputAttribute(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
// TitleBar
static int imgui_BeginNodeTitleBar(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
static int imgui_EndNodeTitleBar(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
// Link
static int imgui_NodeLink(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
    int first = luaL_checknumber(L, 2);
    int second = luaL_checknumber(L, 3);
    ImNodes::Link(id, first, second);
    return 0;
}
// Push Color Style
static int imgui_PushColorStyle(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
    ...
    ImNodes::PushColorStyle(id, IM_COL32(r, g, b, a));
    return 0;
}
// Pop Color Style
static int imgui_PopColorStyle(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
...
// Set Position
static int imgui_SetNodeScreenSpacePos(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
...
static int imgui_SetNodeEditorSpacePos(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
...
static int imgui_SetNodeGridSpacePos(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    int id = luaL_checknumber(L, 1);
...
// Save
static int imgui_SaveCurrentEditorStateToIniString(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 2);
    imgui_NewFrame();
    size_t data_size = 0;
    const char* result = ImNodes::SaveCurrentEditorStateToIniString(&data_size);
    lua_pushstring(L, result);
    lua_pushinteger(L, data_size);
    return 2;
}
static int imgui_SaveCurrentEditorStateToIniFile(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    const char* filename = luaL_checkstring(L, 1);
...
static int imgui_SaveCurrentEditorStateToIniFileWithConfig(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    const char* filename = luaL_checkstring(L, 1);
    const char* config = luaL_checkstring(L, 2);
    const size_t config_size = luaL_checkinteger(L, 3);
    ImNodes::SaveCurrentEditorStateToIniFileWithConfig(filename, config, config_size);
    return 0;
}

// Load
static int imgui_LoadCurrentEditorStateFromIniString(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    const char* data = luaL_checkstring(L, 1);
...
static int imgui_LoadCurrentEditorStateFromIniFile(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 0);
    imgui_NewFrame();
    const char* filename = luaL_checkstring(L, 1);
...
static int imgui_LoadCurrentEditorStateFromIniFileWithConfig(lua_State* L)
{
    DM_LUA_STACK_CHECK(L, 1);
    imgui_NewFrame();
    const char* filename = luaL_checkstring(L, 1);
    const char* result = ImNodes::LoadCurrentEditorStateFromIniFileWithConfig(filename);
    lua_pushstring(L, result);
    return 1;
}

static int imgui_IsLinkCreated(lua_State* L)
{
    ...
    bool created = ImNodes::IsLinkCreated(&started_at_node_id, &started_at_attribute_id, &ended_at_node_id, &ended_at_attribute_id, &created_from_snap);
...
static const luaL_reg Module_methods[] =
{
    ...
    {"begin_node_editor", imgui_BeginNodeEditor},
    {"end_node_editor", imgui_EndNodeEditor},

    {"begin_node", imgui_BeginNode},
    {"end_node", imgui_EndNode},

    {"begin_input_attribute", imgui_BeginInputAttribute},
    {"end_input_attribute", imgui_EndInputAttribute},

    {"begin_output_attribute", imgui_BeginOutputAttribute},
    {"end_output_attribute", imgui_EndOutputAttribute},

    {"begin_node_title_bar", imgui_BeginNodeTitleBar},
    {"end_node_title_bar", imgui_EndNodeTitleBar},

    {"node_link", imgui_NodeLink},

    {"push_node_color_style", imgui_PushColorStyle},
    {"pop_node_color_style", imgui_PopColorStyle},

    {"set_node_pos_screen_space", imgui_SetNodeScreenSpacePos},
    {"set_node_pos_editor_space", imgui_SetNodeEditorSpacePos},
    {"set_node_pos_grid_space", imgui_SetNodeGridSpacePos},

    {"save_current_editor_to_string", imgui_SaveCurrentEditorStateToIniString},
    {"load_current_editor_from_string", imgui_LoadCurrentEditorStateFromIniString},

    {"save_current_editor_to_file", imgui_SaveCurrentEditorStateToIniFile},
    {"save_current_editor_to_file_with_config", imgui_SaveCurrentEditorStateToIniFileWithConfig},
    {"load_current_editor_from_file", imgui_LoadCurrentEditorStateFromIniFile},
    {"load_current_editor_from_file_with_config", imgui_LoadCurrentEditorStateFromIniFileWithConfig},

    {"is_link_created", imgui_IsLinkCreated},
...
static void LuaInit(lua_State* L)
{
    ...
    // IMNODES 
    lua_setfieldstringint(L, "ImNodesCol_NodeBackground", ImNodesCol_NodeBackground);
    lua_setfieldstringint(L, "ImNodesCol_NodeBackgroundHovered", ImNodesCol_NodeBackgroundHovered);
    lua_setfieldstringint(L, "ImNodesCol_NodeBackgroundSelected", ImNodesCol_NodeBackgroundSelected);
    lua_setfieldstringint(L, "ImNodesCol_NodeOutline", ImNodesCol_NodeOutline);
    lua_setfieldstringint(L, "ImNodesCol_TitleBar", ImNodesCol_TitleBar);
    lua_setfieldstringint(L, "ImNodesCol_TitleBarHovered", ImNodesCol_TitleBarHovered);
    lua_setfieldstringint(L, "ImNodesCol_TitleBarSelected", ImNodesCol_TitleBarSelected);
    lua_setfieldstringint(L, "ImNodesCol_Link", ImNodesCol_Link);
    lua_setfieldstringint(L, "ImNodesCol_LinkHovered", ImNodesCol_LinkHovered);
    lua_setfieldstringint(L, "ImNodesCol_LinkSelected", ImNodesCol_LinkSelected);
    lua_setfieldstringint(L, "ImNodesCol_Pin", ImNodesCol_Pin);
    lua_setfieldstringint(L, "ImNodesCol_PinHovered", ImNodesCol_PinHovered);
    lua_setfieldstringint(L, "ImNodesCol_BoxSelector", ImNodesCol_BoxSelector);
    lua_setfieldstringint(L, "ImNodesCol_BoxSelectorOutline", ImNodesCol_BoxSelectorOutline);
    lua_setfieldstringint(L, "ImNodesCol_GridBackground", ImNodesCol_GridBackground);
    lua_setfieldstringint(L, "ImNodesCol_GridLine", ImNodesCol_GridLine);
    lua_setfieldstringint(L, "ImNodesCol_GridLinePrimary", ImNodesCol_GridLinePrimary);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapBackground", ImNodesCol_MiniMapBackground);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapBackgroundHovered", ImNodesCol_MiniMapBackgroundHovered);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapOutline", ImNodesCol_MiniMapOutline);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapNodeBackground", ImNodesCol_MiniMapNodeBackground);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapNodeBackgroundSelected", ImNodesCol_MiniMapNodeBackgroundSelected);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapNodeOutline", ImNodesCol_MiniMapNodeOutline);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapLink", ImNodesCol_MiniMapLink);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapLinkSelected", ImNodesCol_MiniMapLinkSelected);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapCanvas", ImNodesCol_MiniMapCanvas);
    lua_setfieldstringint(L, "ImNodesCol_MiniMapCanvasOutline", ImNodesCol_MiniMapCanvasOutline);
    lua_setfieldstringint(L, "ImNodesCol_COUNT", ImNodesCol_COUNT);

    lua_setfieldstringint(L, "ImNodesStyleVar_GridSpacing", ImNodesStyleVar_GridSpacing);
    lua_setfieldstringint(L, "ImNodesStyleVar_NodeCornerRounding", ImNodesStyleVar_NodeCornerRounding);
    lua_setfieldstringint(L, "ImNodesStyleVar_NodePadding", ImNodesStyleVar_NodePadding);
    lua_setfieldstringint(L, "ImNodesStyleVar_NodeBorderThickness", ImNodesStyleVar_NodeBorderThickness);
    lua_setfieldstringint(L, "ImNodesStyleVar_LinkThickness", ImNodesStyleVar_LinkThickness);
    lua_setfieldstringint(L, "ImNodesStyleVar_LinkLineSegmentsPerLength", ImNodesStyleVar_LinkLineSegmentsPerLength);
    lua_setfieldstringint(L, "ImNodesStyleVar_LinkHoverDistance", ImNodesStyleVar_LinkHoverDistance);
    lua_setfieldstringint(L, "ImNodesStyleVar_PinCircleRadius", ImNodesStyleVar_PinCircleRadius);
    lua_setfieldstringint(L, "ImNodesStyleVar_PinQuadSideLength", ImNodesStyleVar_PinQuadSideLength);
    lua_setfieldstringint(L, "ImNodesStyleVar_PinTriangleSideLength", ImNodesStyleVar_PinTriangleSideLength);
    lua_setfieldstringint(L, "ImNodesStyleVar_PinLineThickness", ImNodesStyleVar_PinLineThickness);
    lua_setfieldstringint(L, "ImNodesStyleVar_PinHoverRadius", ImNodesStyleVar_PinHoverRadius);
    lua_setfieldstringint(L, "ImNodesStyleVar_PinOffset", ImNodesStyleVar_PinOffset);
    lua_setfieldstringint(L, "ImNodesStyleVar_MiniMapPadding", ImNodesStyleVar_MiniMapPadding);
    lua_setfieldstringint(L, "ImNodesStyleVar_MiniMapOffset", ImNodesStyleVar_MiniMapOffset);
    lua_setfieldstringint(L, "ImNodesStyleVar_COUNT", ImNodesStyleVar_COUNT);

    lua_setfieldstringint(L, "ImNodesStyleFlags_None", ImNodesStyleFlags_None);
    lua_setfieldstringint(L, "ImNodesStyleFlags_NodeOutline", ImNodesStyleFlags_NodeOutline);
    lua_setfieldstringint(L, "ImNodesStyleFlags_GridLines", ImNodesStyleFlags_GridLines);
    lua_setfieldstringint(L, "ImNodesStyleFlags_GridLinesPrimary", ImNodesStyleFlags_GridLinesPrimary);
    lua_setfieldstringint(L, "ImNodesStyleFlags_GridSnapping", ImNodesStyleFlags_GridSnapping);

    lua_setfieldstringint(L, "ImNodesPinShape_Circle", ImNodesPinShape_Circle);
    lua_setfieldstringint(L, "ImNodesPinShape_CircleFilled", ImNodesPinShape_CircleFilled);
    lua_setfieldstringint(L, "ImNodesPinShape_Triangle", ImNodesPinShape_Triangle);
    lua_setfieldstringint(L, "ImNodesPinShape_TriangleFilled", ImNodesPinShape_TriangleFilled);
    lua_setfieldstringint(L, "ImNodesPinShape_Quad", ImNodesPinShape_Quad);
    lua_setfieldstringint(L, "ImNodesPinShape_QuadFilled", ImNodesPinShape_QuadFilled);

    lua_setfieldstringint(L, "ImNodesMiniMapLocation_BottomLeft", ImNodesMiniMapLocation_BottomLeft);
    lua_setfieldstringint(L, "ImNodesMiniMapLocation_BottomRight", ImNodesMiniMapLocation_BottomRight);
    lua_setfieldstringint(L, "ImNodesMiniMapLocation_TopLeft", ImNodesMiniMapLocation_TopLeft);
    lua_setfieldstringint(L, "ImNodesMiniMapLocation_TopRight", ImNodesMiniMapLocation_TopRight);
