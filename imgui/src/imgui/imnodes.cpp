...
void SaveCurrentEditorStateToIniFileWithConfig(const char* const file_name, const char* const config, const size_t config_size)
{
    SaveEditorStateToIniFileWithConfig(&EditorContextGet(), file_name, config, config_size);
}

void SaveEditorStateToIniFileWithConfig(const ImNodesEditorContext* const editor, const char* const file_name, const char* const config, const size_t config_size)
{
    size_t      data_size = 0u;
    const char* data = SaveEditorStateToIniString(editor, &data_size);
    size_t size_to_save = data_size + config_size + 1;
    char data_to_save[size_to_save];
    strcpy(data_to_save, data);
    strcat(data_to_save, config);
    FILE*       file = ImFileOpen(file_name, "wt");
    if (!file)
    {
        return;
    ...
    fwrite(data_to_save, sizeof(char), size_to_save, file);
    fclose(file);
}
...
const char* LoadCurrentEditorStateFromIniFileWithConfig(const char* const file_name)
{
    return LoadEditorStateFromIniFileWithConfig(&EditorContextGet(), file_name);
}

const char* LoadEditorStateFromIniFileWithConfig(ImNodesEditorContext* const editor, const char* const file_name)
{
    size_t data_size = 0u;
    char*  file_data = (char*)ImFileLoadToMemory(file_name, "rb", &data_size);
